...
extension UIViewController {
    ...
    @discardableResult public func setRightButtonItem(title: String, actionBlock: ((UIButton) -> Void)?) -> UIButton {
        return setRightButtonItem(frame: CGRect(x: 0, y: 0, width: 44, height: 44), title: title, image: nil, actionBlock: actionBlock)
    }
    ...
    @discardableResult public func setRightButtonItem(image: UIImage?, actionBlock: ((UIButton) -> Void)?) -> UIButton {
        return setRightButtonItem(frame: CGRect(x: 0, y: 12, width: 20, height: 20), title: nil, image: image, actionBlock: actionBlock)
    ...
    public func setRightButtonItem(titles: [String], actionBlock: ((Int) -> Void)?) {
...
//            items.append(negativeSpacer)
    ...
    ///   - images: 图片数组
    ///   - actionBlock: 点击回调
    public func setRightButtonItem(images: [UIImage?], actionBlock: ((Int) -> Void)?) {
        assert(images.count > 0, "请传入非空的标题数组")
        
        var items: [UIBarButtonItem] = [UIBarButtonItem]()
        for (index, image) in images.enumerated() {
            
            let btn = buttomItem(frame: CGRect(x: 0, y: 12, width: 20, height: 20), title: nil, image: image)
    ...
    ///   - images: 图片数组
    ///   - actionBlock: 点击回调
    public func setLeftButtonItem(images: [UIImage?], actionBlock: ((Int) -> Void)?) {
        var items: [UIBarButtonItem] = [UIBarButtonItem]()
        for (index, image) in images.enumerated() {
            let btn = buttomItem(frame: CGRect(x: 0, y: 12, width: 20, height: 20), title: nil, image: image)
...
//            items.append(negativeSpacer)
    ...
    ///   - titles: 标题数组
    ///   - actionBlock: 点击事件回调，多个按钮，通过tag区分
    public func setLeftButtonItem(titles: [String], actionBlock: ((Int) -> Void)?) {
        assert(titles.count > 0, "请传入非空的标题数组")
